...
// Software Serial for communication with slave Arduino
SoftwareSerial slaveSerial(12, A1); // RX=12, TX=A1
...
const int RESET_BUTTON_PIN = 3; // New reset button pin
...
bool buttonPressed = false; // New variable to track if button was already pressed
...
void setup() {
  Serial.begin(9600);
  slaveSerial.begin(9600); // Initialize Software Serial communication

  // Button setup
  pinMode(START_BUTTON_PIN, INPUT_PULLUP);
  pinMode(RESET_BUTTON_PIN, INPUT_PULLUP); // New reset button setup
...
void loop() {
  handleButtonInput();
  handleResetButton(); // New function call for reset button
...
void detectColor() {
  ...
  if (greenFreq < redFreq && greenFreq < blueFreq && greenFreq > 0) {
    ...
    Serial.println("GREEN BOTTLE DETECTED - SENDING REJECT COMMAND");
...
void handleBottleCounting() {
  ...
  if (currentIRReading == LOW && lastIRReading == HIGH &&
    ...
    if (bottleCount >= TARGET_BOTTLES) {
      ...
      Serial.print("TARGET REACHED - SENDING DISPENSE COMMAND ONLY (");
      Serial.print(TARGET_BOTTLES);
      Serial.println(" bottles)");
      
      // For the 3rd bottle, send DISPENSE command directly (no counter increment)
      // This will move both MG996R to 0° and SG90 to 90°
      sendServoCommand(SERVO_DISPENSE_CMD);
    } else {
      // For bottles 1 and 2, send counter increment command
      Serial.print("BOTTLE COUNTED - SENDING COUNTER INCREMENT COMMAND (Count: ");
