...
const int SERVO_REJECT_ANGLE = 90;  // MG996R works better at 90 degrees
...
void handleBottleCounting() {
  ...
  if (currentIRReading == LOW && lastIRReading == HIGH &&
    ...
    // Activate servo when count reaches 3 or 5
    if (bottleCount == 3 || bottleCount == 5) {
      activateDispenseServo();
      Serial.print("COUNT MILESTONE REACHED: ");
      Serial.print(bottleCount);
      Serial.println(" bottles - Activating MG996R servo...");
    }
...
void updateLCD() {
  if (!systemReady) {
    ...
    lcd.setCursor(0, 1);
    lcd.print("Select Mode...  ");
  } else if (taskCompleted && !isDispensing && !servoDelaying && !servoActive) {
    // Display thank you message when task is completed and servo action is done
    lcd.setCursor(0, 0);
    lcd.print("Thank you for   ");
    lcd.setCursor(0, 1);
    lcd.print("using ECOBOT!   ");
  } else if (isDispensing || (servoDelaying || servoActive)) {
    // Display dispensing message when servo is active (at count 3 or 5)
    lcd.setCursor(0, 0);
    lcd.print("Count: ");
...
void activateDispenseServo() {
  if (!servoActive && !servoDelaying) {
    servoDelayTime = millis();
    servoDelaying = true;
    isDispensing = true; // Set dispensing flag for LCD display
    Serial.print("MILESTONE REACHED (");
    Serial.print(bottleCount);
...
void handleServoMovement() {
  ...
  if (servoDelaying && (millis() - servoDelayTime) >= SERVO_DELAY_DURATION) {
    ...
    rejectServo.write(SERVO_REJECT_ANGLE); // Rotate to 90 degrees (better for MG996R)
    servoActionTime = millis();
    servoActive = true;
    servoDelaying = false;
    
    if (isDispensing) {
      Serial.print("MG996R SERVO ACTIVATED - Dispensing at count ");
      Serial.print(bottleCount);
      Serial.println("!");
    } else {
  ...
  if (servoActive && (millis() - servoActionTime) >= SERVO_ACTIVE_DURATION) {
    ...
    if (taskCompleted) {
      Serial.println("MG996R SERVO RETURNED - Dispensing complete!");
      // Optional: Reset system for next use
      // Uncomment the lines below if you want to automatically reset after completion
      // systemReady = false;
      // taskCompleted = false;
      // bottleCount = 0;
    } else {
