...
const char* bottleCountEndpoint = "/api/bottle-count"; // New endpoint for bottle count notifications
...
void sendBottleCountCompleteNotification() {
  if (!wifiConnected) {
    Serial.println("Cannot send bottle count notification - WiFi not connected");
    return;
  }
  
  Serial.println("Sending bottle count complete notification to web app...");
  
  // Create JSON payload
  String jsonPayload = "{";
  jsonPayload += "\"count\":" + String(TARGET_BOTTLES) + ",";
  jsonPayload += "\"status\":\"complete\",";
  jsonPayload += "\"message\":\"3 bottles have been accepted and processed\",";
  jsonPayload += "\"timestamp\":\"" + String(millis()) + "\",";
  jsonPayload += "\"device_id\":\"ecobot_001\"";
  jsonPayload += "}";
  ...
  // Make HTTP POST request
  httpClient.beginRequest();
  httpClient.post(bottleCountEndpoint);
  httpClient.sendHeader("Content-Type", "application/json");
  httpClient.sendHeader("Content-Length", jsonPayload.length());
  httpClient.beginBody();
  httpClient.print(jsonPayload);
  ...
  Serial.print("Bottle Count HTTP Status Code: ");
  Serial.println(statusCode);
  Serial.print("Response: ");
  Serial.println(response);
  
  if (statusCode == 200 || statusCode == 201) {
    Serial.println("Bottle count notification sent successfully!");
  } else {
    Serial.println("Failed to send bottle count notification");
  }
}
...
void handleBottleCounting() {
  ...
  if (currentIRReading == LOW && lastIRReading == HIGH &&
    ...
    if (bottleCount >= TARGET_BOTTLES) {
      ...
      // Send notification to web app that 3 bottles have been accepted
      sendBottleCountCompleteNotification();
