...
const char* serverAddress = "ecobot-idnm.onrender.com"; 
const int serverPort = 443;
const char* notificationEndpoint = "/api/bin-status"; 
...
const unsigned long NOTIFICATION_COOLDOWN = 60000; 
...
void setup() {
  ...
  Serial.println("- WiFi notifications");
...
void checkContainerLevel() {
  ...
  if (containerFull && !wasFull) {
    Serial.println("CONTAINER FULL - Sending notification");
    sendNotification("full", "Container is full - please empty");
  } else if (!containerFull && wasFull) {
    Serial.println("CONTAINER EMPTY - Sending notification");
    sendNotification("empty", "Container is now empty - ready for use");
  }
...
void sendNotification(const char* status, const char* message) {
  if (!wifiConnected) {
