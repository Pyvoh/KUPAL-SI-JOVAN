...
void detectColor() {
  ...
  // Detect GREEN (reject bottle)
  if (greenFreq < redFreq && greenFreq < blueFreq) {
    colorStatus = "GREEN-REJECT";
    shouldCount = false; // Don't count green bottles
  }
...
void handleBottleCounting() {
  ...
  // If green is detected and IR triggers, show rejection
  else if (currentIRReading == LOW && lastIRReading == HIGH &&
           !shouldCount &&
           (millis() - lastCountTime) > COUNT_DELAY) {
    lastCountTime = millis();
    
    // Optional: Print debug info for rejected bottles
    Serial.print("Bottle rejected! Color: ");
