...
void handleBottleCounting() {
  ...
  if (currentIRReading == LOW && lastIRReading == HIGH &&
    ...
    // Send counter increment command to slave when bottle count increases
    ...
    if (bottleCount >= TARGET_BOTTLES) {
      ...
      Serial.print("TARGET REACHED - SENDING DISPENSE COMMAND (");
      Serial.print(TARGET_BOTTLES);
      Serial.println(" bottles)");
      
      // Wait a moment before sending dispense command
      delay(1000);
